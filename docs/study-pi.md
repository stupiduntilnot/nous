...
## 7) Session Branching（会话分叉）是什么，为什么有用，pi-mono 如何做

### 7.1 是什么

`Session Branching` 指的是：在历史会话中的某个节点“分叉”出新路径继续对话，而不是把所有历史线性拼接在一起。

核心数据结构是树：
1. 每条会话记录有 `id` 与 `parentId`。
2. `leaf` 指针表示“当前所在分支末端”。
3. 上下文构建时只取 `root -> leaf` 这条路径，而不是整棵树全部节点。

### 7.2 为什么有用

1. 避免语义污染：
- 同一个问题的 A/B 两条方案不会互相污染到同一上下文。
2. 使上下文可控：
- 只喂当前分支路径，避免把不相关分支一起发给模型。
3. 让回放与调试可追溯：
- 能明确“当前回答来自哪条分支、从哪个节点分出来”。
4. 支持“从历史点重试”：
- 回到历史节点后继续探索，不必重写/复制整段 transcript。

### 7.3 pi-mono 怎么做（要点）

pi-mono 的分叉是**显式触发**，不是模型自动触发：

1. `/fork`（或 RPC `fork`）
- 从选定用户消息节点分叉。
- 语义是创建一个“新 session 文件”，并把目标路径导出为新会话继续工作。

2. `/tree`
- 在同一个 session 文件中移动 `leaf` 到目标节点（可选生成 branch summary）。
- 不新建文件，直接切换当前活跃分支路径。

3. 普通 `prompt`
- 默认只是追加到当前 `leaf` 下，不会隐式创建新分支。

4. 上下文构建
- 只基于当前 `leaf` 的分支路径构建 `buildSessionContext()`。
- 不会把整棵树所有节点都塞进模型上下文。

结论：
1. 分叉决策来自用户/调用方命令（`fork/tree`），而不是 agent 自行猜测。
2. 这既保证可预测性，也保证上下文不会混入“其他分支”的历史。
...
6. Session 必须支持“分支路径上下文”语义：
- 上下文默认只来自 active `leaf` 路径，而不是整棵 session 树。
7. 分叉行为必须显式命令触发（如 `branch_session` / `fork` / `tree`），避免隐式自动分叉导致行为不可预测。
