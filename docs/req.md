...
6. 开发成本优先：开发阶段默认使用 OpenAI provider。
...
### 5.2 Session Branching 语义（强约束）

1. Session 必须支持树结构语义（至少具备 `id` + `parent_id`/等价关系），而不是仅线性 transcript。
2. 系统必须有 `active leaf` 概念，表示当前会话分支位置。
3. 构建模型上下文时，默认只使用 `root -> active leaf` 路径，不得默认混入其他分支节点。
4. 分叉必须是显式触发行为（如 `branch_session`、fork/tree 等），不能由模型输出隐式决定。
5. 普通 `prompt` 默认追加到当前 active leaf，不隐式创建新分支。
6. `branch_session`/分叉操作后，后续上下文应切到新分支路径并保持可重建。
7. 需要支持从历史节点继续探索的稳定行为：
- 从某历史节点分叉后，原分支仍可回放，新分支独立推进。
8. Session 恢复/切换后，active leaf 与对应上下文路径必须可确定且可重放。
9. 分支语义与 TUI 解耦：
- TUI 只负责触发分叉命令与展示，不承担分支状态计算。
